...
import { SlidersHorizontal, ArrowUpDown, RefreshCw, Search, Smile } from "lucide-react";
...
export default function ListingView({
  ...
  const [selectedCollection, setSelectedCollection] = useState(initialCollection || "all");
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      if (selectedMetal !== "all") {
        const matchesMetal = product.metals.some(m => m.toLowerCase().includes(selectedMetal.toLowerCase()));
      ...
      if (searchText.trim()) {
        ...
        const matchesCol = product.collection?.toLowerCase().includes(query) || false;
        if (!matchesName && !matchesDesc && !matchesCat && !matchesCol) return false;
      }
  ...
  }, [products, selectedCategory, selectedCollection, selectedMetal, maxPrice, onlyCanadianMined, sortOption, searchText]);

  // Reset page limit to 6 on any filter, sort, or search change
  useEffect(() => {
    setVisibleCount(6);
    setIsFetchingMore(false);
  }, [selectedCategory, selectedCollection, selectedMetal, maxPrice, onlyCanadianMined, sortOption, searchText]);
  ...
  return (
    <div className="listing-container">
      ...
      <div className="listing-header">
        <span className="listing-sub">
          FINE JEWELRY & HEIRLOOMS
        </span>
        <h1 className="listing-title">
          {selectedCategory !== "all" ? `${selectedCategory} Collection` : selectedCollection !== "all" ? `${selectedCollection} Signature Line` : "Paris Jewellers Catalogue"}
        </h1>
        <p className="listing-desc">
          Browse our stunning assortments. All our diamonds are hand-selected, certified ethical, customizable in fine white gold, yellow gold or romantic rose gold, and backed by a lifetime of professional showroom care.
        </p>

        {searchText && (
          <div className="search-badge-notice">
            <Search className="w-4 h-4" style={{ color: "#c3a475" }} />
            <span>Search results for: <strong>"{searchText}"</strong> ({filteredProducts.length} items found)</span>
            <button onClick={() => setSearchText("")} className="search-badge-clear">
              Clear
            </button>
          </div>
        )}
        ...
        <aside className="sidebar-filters-panel">
          <div className="filter-top-bar">
            <h3 className="filter-heading">
              <SlidersHorizontal className="w-4 h-4" style={{ color: "#c3a475" }} /> Filters
            </h3>
            <button 
          ...
          <div className="filter-group">
            ...
            <div className="filter-buttons-list">
              {["all", "engagement", "wedding", "necklaces", "earrings", "bracelets"].map((cat) => (
                <button
                  key={cat}
          ...
          <div className="filter-group">
            ...
            <div className="filter-buttons-list">
              {["all", "glow", "disney", "canadian", "chantilly", "serendipity"].map((col) => (
                <button
                  key={col}
          ...
          <div className="filter-group">
            ...
            <div className="metals-bubble-grid">
              {["all", "White Gold", "Yellow Gold", "Rose Gold", "Platinum"].map((metal) => (
                <button
                  key={metal}
        ...
        <main className="listing-main-content">
          ...
          <div className="listing-top-controls">
            <div className="showing-count-txt">
              Showing <strong className="font-bold">{Math.min(visibleCount, filteredProducts.length)}</strong> of <strong className="font-bold">{filteredProducts.length}</strong> luxurious designs
            </div>

            {/* Sorters */}
            <div className="sorters-group">
              <div className="sorters-label">
                <ArrowUpDown className="w-3.5 h-3.5" style={{ color: "#c3a475" }} /> Sort:
              </div>
              <select
                value={sortOption}
          ...
          {filteredProducts.length > 0 ? (
            <>
              <div className="grid-responsive-wrap">
                ...
                {isFetchingMore && Array.from({ length: Math.min(3, filteredProducts.length - visibleCount) }).map((_, idx) => (
                  <div key={`skeleton-${idx}`} className="skeleton-product-card" id={`skeleton-placeholder-${idx}`}>
              ...
              {(visibleCount < filteredProducts.length || isFetchingMore) && (
                <div ref={observerRef} className="auto-loader-sentinel" id="listing-lazy-loader">
                  ...
                  <p className="loader-text-accent">
                    {isFetchingMore ? "Unveiling further exquisite handcrafts..." : "Loading more exceptional designs..."}
                  </p>
                </div>
              )}
            </>
            ...
            <div className="not-found-card">
              ...
              <p className="not-found-desc">
                We couldn’t find any jewelry design matching your active selection set. Please try lowering your price thresholds, choosing another metal, or resetting filters.
              </p>
              <button 
                onClick={handleResetFilters}
